...
TravisClient.prototype._authenticateAccessToken = function (token, callback) {
    this._get('/users', {
        access_token: token
    }, function (err) {
        if (!err) {
...
TravisClient.prototype._authenticateGithubToken = function (token, callback) {
    this.auth.github({
        github_token: token
    }, function (err, res) {
        if (err) { return callback(err); }

        this._authenticateAccessToken(res.access_token, callback);
    }.bind(this));
};

TravisClient.prototype._authenticateBasic = function (username, password, callback) {
    var GITHUB_TRAVIS_TOKEN_ID = 2362358;
    var GITHUB_TRAVIS_PRO_TOKEN_ID = 2442020;
    ...
    github.authorization.getAll({}, function (err, res) {
        var app = _.findWhere(res, {
            id: this.pro ? GITHUB_TRAVIS_PRO_TOKEN_ID : GITHUB_TRAVIS_TOKEN_ID
        });
        ...
        this._authenticateGithubToken(app.token, callback);
    }.bind(this));
};
...
TravisClient.prototype.authenticate = function (msg, callback) {
    ...
    if (_.difference(_.keys(msg), ['username', 'password']).length === 0) {
        return this._authenticateBasic(msg.username, msg.password, callback);
    } else if (_.difference(_.keys(msg), ['access_token']).length === 0) {
        return this._authenticateAccessToken(msg.access_token, callback);
    } else if (_.difference(_.keys(msg), ['github_token']).length === 0) {
        return this._authenticateGithubToken(msg.github_token, callback);
    } else {
